...
public partial class AppDbContext
{
    ...
    // Immutability enforcement and audit capture are async (query the DB for ETR context).
    // Nothing in this codebase calls the sync SaveChanges — throw instead of blocking a thread
    // on GetAwaiter().GetResult(), so any future accidental sync caller fails loudly rather than
    // either deadlocking or silently skipping immutability/audit enforcement.
    public override int SaveChanges()
    {
        throw new NotSupportedException($"{nameof(AppDbContext)} does not support synchronous SaveChanges — use SaveChangesAsync instead.");
    }
    ...
    /// covers both explicit AuditLog entries created by services (EtrService etc.) and the
    /// ones auto-generated by <see cref="CapturePendingAuditEntries"/> below.</summary>
    private void StampAuditRequestMetadata()
    {
        var ipAddress = _currentUserService?.IPAddress;
        var userAgent = _currentUserService?.UserAgent;
        if (ipAddress == null && userAgent == null) return;

        foreach (var entry in ChangeTracker.Entries<AuditLog>().Where(e => e.State == EntityState.Added))
        {
            if (string.IsNullOrEmpty(entry.Entity.IPAddress)) entry.Entity.IPAddress = ipAddress;
            if (string.IsNullOrEmpty(entry.Entity.UserAgent)) entry.Entity.UserAgent = userAgent;
        }
    }
    ...
    private static int GetPrimaryKeyValue(EntityEntry entry)
    {
        ...
        // Composite key (currently only CourseSubject: CourseId+SubjectId) — AuditLog.RecordId is
        // a single int, so both parts can't be losslessly encoded. Use the first key property's
        // value so RecordId is at least a real, traceable ID instead of always 0; the full
        // composite key is still available in the OldValue/NewValue JSON payload.
