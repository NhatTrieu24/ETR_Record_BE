...
- **AmendmentRequest** *(mới — không có trong bản trước của tài liệu này)*: `AmendmentRequestId, SubjectResultId, RequestedByAccountId, Reason, OldValue, NewValue, Status (Pending/Approved/Rejected), ApprovedByAccountId, ApprovedAt, DecisionComment` — yêu cầu "mở khóa" 1 `SubjectResult` đã Sign-off, để Instructor sửa lại mà không cần Unlock toàn bộ ETRCourseRecord
...
5. Sau Sign-off, nếu phát hiện sai sót: **không được sửa trực tiếp** — phải qua luồng Amendment/Unlock-request, **đã có API**: Instructor/Admin tạo yêu cầu qua `POST /subjectsignoff/{subjectResultId}/unlock-request` → TrainingManager/Admin xét duyệt qua `POST /amendments/{id}/approve` (reset `SubjectResult` về "Pending", vô hiệu hóa chữ ký cũ) hoặc `POST /amendments/{id}/reject` (giữ nguyên trạng thái, bắt buộc `Comment` giải trình).
...
- `PUT /evidences/{id}/verify` — verify từng cái một
- `PUT /evidences/bulk-verify` *(mới — không có trong bản trước của tài liệu này, xem 6.7)*
- `DELETE /evidences/{id}`

**SubjectSignoffController**: `GET/POST /subjectsignoff`, `GET /subjectsignoff/check/{subjectResultId}`, `POST /subjectsignoff/{subjectResultId}/unlock-request` *(mới — Instructor/Admin tạo Amendment Request, xem `AmendmentRequest` ở 6.3)*

**AmendmentsController** *(mới — không có trong bản trước của tài liệu này)*: `GET /amendments`, `GET /amendments/{id}`, `POST /amendments/{id}/approve`, `POST /amendments/{id}/reject` — TrainingManager/Admin xét duyệt Amendment Request
...
- **Nghẽn cổ chai QA verify Evidence**: đã được giải quyết — `PUT /evidences/bulk-verify` cho phép verify/reject nhiều `EvidenceFile` trong 1 lần gọi thay vì 90 lần gọi riêng lẻ cho 1 lớp 30 học viên × 3 file.
- **Instructor Sign-off nhầm rồi phát hiện chấm sai**: đã có API unlock cấp `SubjectSignoff` riêng biệt (`AmendmentRequest` + `SubjectSignoffController.unlock-request` + `AmendmentsController.approve/reject`) — không còn phải mở khóa toàn bộ ETRCourseRecord (Module 4) để sửa 1 Subject nữa.
