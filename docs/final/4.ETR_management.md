...
| **Academic Staff / Instructor** | Liên kết dữ liệu, theo dõi tiến độ, Submit ETR — `POST /etr/{id}/submit` cho phép cả 2 role (`[Authorize(Roles = "Instructor,Admin,Academic")]`) |
| **QA Staff** | Verify ETR |
...
- `GET /etr/due-for-training?courseId=&daysThreshold=30` — như `expiring-students` nhưng quét **toàn bộ Course** nếu không truyền `courseId` (dùng chung `GetExpiringStudentsAsync`, không viết lại logic Expired/ExpiringSoon)
- `POST /etr/refresh-grounded-status` *(Admin-only)* — quét toàn bộ `UserProfile` Active/Grounded, tự động set/clear `Grounded` dựa trên chứng chỉ hết hạn; là cơ chế trigger **thủ công** (gọi tay hoặc cron ngoài hệ thống), không có background job tự chạy trong scope hiện tại

## 4.7 Edge cases / Exception flows

- **Grounded status — đã có API, vẫn cần trigger thủ công**: `POST /etr/refresh-grounded-status` (Admin) đã đóng gap "Learner hết hạn chứng chỉ không tự chuyển trạng thái" nêu ở bản trước của tài liệu này — nhưng đây KHÔNG phải cơ chế tự động theo thời gian thực; nếu không có cron/scheduler bên ngoài gọi định kỳ, `UserProfile.Status` có thể "trễ" so với `ExpiryDate` thực tế cho đến lần gọi thủ công tiếp theo.
- **Unlock 1 ETR record đã Completed để sửa 1 lỗi nhỏ ở 1 Subject**: **đã có API cấp Subject** — `SubjectSignoffController: POST /subjectsignoff/{subjectResultId}/unlock-request` (Instructor/Admin tạo yêu cầu) → `AmendmentsController: POST /amendments/{id}/approve|reject` (TrainingManager/Admin xét duyệt) — Approve reset riêng `SubjectResult` đó về "Pending" và vô hiệu hóa chữ ký cũ, không cần `reopen` toàn bộ ETRCourseRecord nữa. Xem Module 6.
- **Recurrent Training**: `PreviousRecordId` được tự động liên kết khi Enroll lại — `EnrollmentService.CreateEnrollmentAsync` set `PreviousRecordId = previousEtr.ETRCourseRecordId` cho `ETRCourseRecord` mới nếu Learner có ETR cũ (đã hết hạn) cho cùng Course. Luồng "học lại" đã được nối chuỗi lịch sử, không còn là gap.
