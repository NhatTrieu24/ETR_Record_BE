...
| Actor | Quyền hạn |
|---|---|
| **Instructor** | Tạo Session, ghi nhận Attendance Record, confirm session |
...
- **Session**: `SessionId, ClassId, SubjectId, SessionTitle, SessionDate, Location, IsConfirmed, ConfirmedByAccountId, ConfirmedAt, IsAssessmentRequired, IsChecklistRequired`
- **AttendanceRecord**: `AttendanceRecordId, SessionId, ClassStudentId, Status (Present/Absent/Late), Remarks, RecordedByAccountId, RecordedAt`

## 5.4 Business Rules

1. Session phải thuộc 1 Class cụ thể và 1 Subject cụ thể (không điểm danh tự do không gắn Subject).
2. Mỗi ClassStudent trong Session chỉ có đúng 1 AttendanceRecord (không duplicate).
3. `Status = Late/Absent` nên đi kèm `Remarks` (lý do) — khuyến nghị bắt buộc ở tầng validation khi Status ≠ Present.
...
```mermaid
flowchart TD
    A[Instructor tạo Session cho Class+Subject] --> B[Điểm danh từng ClassStudent: Present/Absent/Late + Remark]
...
**SessionsController**: `GET/POST/PUT/DELETE /sessions`, `GET /sessions/{id}`
...
- `GET /attendance/student/{classStudentId}` — lịch sử điểm danh của 1 học viên
