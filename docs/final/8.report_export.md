...
> `GET /dashboard/my-dashboard` đã được mở rộng đáng kể so với bản trước của tài liệu này — mỗi role hiện nhận thêm widget riêng (Admin: `systemStats`; TrainingManager/Auditor: `monthlyTrend`; Academic: `expiringStudentsCount`; Auditor: `lockedRecords`/`recentLockedEtrs`/`recentAuditLogs`/`recentExportJobs`; QA: `evidenceSummary`/`recentEvidenceFiles`; Instructor: `todaySessions`/`pendingSignoffs`; Student: `profile`/`certificateSummary`) chỉ trong 1 lần gọi duy nhất. Chi tiết đầy đủ schema từng role: `docs/maintain/2026-08-16_feature-export-list-va-mo-rong-dashboard-theo-role.md`.

**SearchController**: `GET /search/classes`, `GET /search/etrs`

**ExportsController**
- `GET /exports?page=&pageSize=` *(mới — không có trong bản trước của tài liệu này)* — danh sách export job đã thực hiện, phân trang; khắc phục số "Audit Packages Exported" ở Dashboard/FE trước đây bị tính ảo do không có nguồn dữ liệu thật
